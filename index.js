...
const uploadFileViaSocket = async (fileUri, fileName, fileType, messageType) => {
  try {
    ...
    console.log("🚀 File read successfully, size:", fileData.length, "characters");
    ...
    console.log("🚀 Sending upload payload with keys:", Object.keys(uploadPayload));
...
socket.on("initiate_call", (data) => {
  ...
  activeCalls.forEach((call, existingCallId) => {
    if (call.participants.includes(caller_id) || call.participants.includes(recipient_id)) {
      // Remove calls that are older than 2 minutes or have failed status
      const callAge = new Date() - call.start_time;
      if (callAge > 120000 || call.status === 'failed' || call.status === 'declined') {
  ...
  const callerExistingCall = Array.from(activeCalls.values()).find(
    call => call.participants.includes(caller_id) && (call.status === 'active' || call.status === 'ringing')
  );
  ...
  const recipientExistingCall = Array.from(activeCalls.values()).find(
    call => call.participants.includes(recipient_id) && (call.status === 'active' || call.status === 'ringing')
  );

  if (recipientExistingCall) {
    console.log(`❌ Recipient ${recipient_id} is busy in call: ${recipientExistingCall.callId} (status: ${recipientExistingCall.status})`);
    socket.emit("call_failed", {
      callId,
      error: "User is busy",
      reason: "user_busy"
  ...
  if (recipient) {
    ...
    console.log(`📞 Call sent to online recipient ${recipient_id} (socket: ${recipient.socketId})`);
  } else {
    // Recipient is offline - send FCM push notification
    console.log(`🔕 Recipient ${recipient_id} offline, sending FCM push...`);
    ...
    con.query(
      ...
      async (err, results) => {
        ...
        if (results.length === 0) {
          console.log(`❌ No user found for recipient_id ${recipient_id}`);
          return;
        }
        ...
        const title = callType === 'audio' ? 'Incoming voice call' : 'Incoming video call';
...
socket.on("end_call", (data) => {
  ...
  const duration = call.answer_time ? 
    Math.floor((endTime - call.answer_time) / 1000) : 0;
...
socket.on("webrtc_offer", (data) => {
  ...
  if (recipient) {
    ...
    console.log(`❌ Recipient ${recipientId} not found for WebRTC offer`);
  }
});
...
socket.on("webrtc_ice_candidate", (data) => {
  ...
  if (otherUser) {
    ...
    console.log(`❌ Other user ${otherUserId} not found for ICE candidate`);
  }
});
...
socket.on("get_call_status", (data) => {
  ...
  socket.on("upload_file", async (uploadData) => {
      ...
      console.log("📁 File data length:", uploadData.fileData ? uploadData.fileData.length : 'undefined');
...
socket.on("mark_message_read", (data) => {
  ...
  saveMessageStatus(messageId, userId, 'read', (err) => {
    ...
    con.query(query, [messageId], (err, results) => {
      ...
      const sender = activeUser.find(user => user.id === message.sender_id);
      if (sender) {
  ...
  socket.on("disconnect", () => {
    ...
    if (disconnectedUser) {
      console.log('📞 Checking for active calls for disconnected user:', disconnectedUser.userId);
      
      // Find any active calls for this user
      const userActiveCalls = Array.from(activeCalls.entries()).filter(([callId, call]) => 
        call.participants.includes(disconnectedUser.userId)
      );

      userActiveCalls.forEach(([callId, call]) => {
        // Get other participant
        const otherUserId = call.participants.find(id => id !== disconnectedUser.userId);
        const otherUser = getActiveUser(otherUserId);
      ...
      if (userActiveCalls.length > 0) {
        console.log(`📞 Cleaned up ${userActiveCalls.length} calls for disconnected user`);
      }
    }
  ...
  socket.on("cleanup_my_calls", (data) => {
    ...
    console.log(`🧹 Manually cleaned ${cleanedCount} calls for user ${userId}`);
  ...
  socket.on("debug_active_calls", () => {
    console.log("🐛 Active calls debug:", Array.from(activeCalls.entries()).map(([id, call]) => ({
      callId: id,
...
setInterval(() => {
  ...
  staleCalls.forEach(callId => {
    const call = activeCalls.get(callId);
    if (call) {
      console.log(`🧹 Periodic cleanup: Removing stale call ${callId} (status: ${call.status}, age: ${now - call.start_time}ms)`);
